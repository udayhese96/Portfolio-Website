  ...
  const skills = [
    {
      ...
      title: "Languages",
      description: "JavaScript, TypeScript, Python, Java, HTML, CSS",
      color: "from-cyan-400 to-blue-500"
    },
    {
      icon: <Code className="w-6 h-6" />,
      title: "Frameworks",
      description: "React, Node.js, Django, Flask, TensorFlow, PyTorch",
    ...
    },
    {
      ...
      title: "Tools",
      description: "Git, Docker, Power BI, Tableau, VS Code",
              ...
              <div className="relative z-10 space-y-6">
                ...
                <div className="grid grid-cols-4 sm:grid-cols-5 md:grid-cols-6 gap-6">
                  {[
                    ...
                    { src: "https://skillicons.dev/icons?i=opencv", alt: "OpenCV" },
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3">
              {skills.map((skill, index) => (
                <div key={index} className="group">
                  <div className="cyber-card magnify-on-hover p-3 h-full">
                    <div className={`w-10 h-10 rounded-lg bg-gradient-to-br ${skill.color} p-2 mb-2 text-white flex items-center justify-center group-hover:scale-110 transition-transform duration-300`}>
                      {skill.icon}
                    </div>
                    <h3 className="text-cyan-300 font-semibold mb-1 font-orbitron text-xs">
                      {skill.title}
                    </h3>
                    <p className="text-cyan-200/60 text-[10px] leading-tight">
                      {skill.description}
                    </p>
                  </div>
                </div>
          ...
          <div className="space-y-6">
            {experienceData.map((experience) => (
              <div key={experience.id} className="cyber-card magnify-on-hover p-6">
                ...
                {/* GitHub Links */}
                {(experience as any).githubLinks && (
                  <div className="flex flex-wrap gap-2 mt-3">
                    {(experience as any).githubLinks.map((link: any, index: number) => (
                      <a
                        key={index}
                        href={link.url}
                        target="_blank"
                        rel="noopener noreferrer"
                        className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-cyan-400/5 border border-cyan-400/40 rounded-lg text-cyan-300 text-xs hover:bg-cyan-400/10 hover:border-cyan-400/60 transition-all duration-300"
                      >
